use windows::{core::PCWSTR, Win32::{Foundation::HANDLE, Security::SECURITY_ATTRIBUTES, System::Threading::CreateEventW}};

use super::WinToIoError;
...
impl Event {
    ...
    pub fn new_raw(init_state: bool, reset: Reset, security: Option<Security>, name: Option<&str>)->std::io::Result<Event> {
        ...
        let handle = unsafe {
            CreateEventW(secref, reset.as_bool(), init_state, winname).conv_io_err()?
        };
        Ok(Event{
            handle: handle,
            reset: reset,
